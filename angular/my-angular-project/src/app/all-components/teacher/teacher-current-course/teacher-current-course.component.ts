...
@Component({
  selector: 'app-teacher-current-course',
  templateUrl: './teacher-current-course.component.html',
  styleUrls: ['./teacher-current-course.component.css']
})
export class TeacherCurrentCourseComponent {
   courses: Coursim[] = [];
    student: Student | any; 
  ...
  minutes = this.currentDate.getMinutes().toString().padStart(2, '0');
    ...
    EndCourse() {
      ...
      this.studentService.ModifyCourse(this.courseFormData).subscribe((mod: any) => {
        ...
        if (mod) {
          alert('Merci !');
          this.router.navigate(['/teacher-historic', this.courseFormData.IDTeacher]);
        } 
        else {
